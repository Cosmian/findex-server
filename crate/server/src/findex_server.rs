...
///
/// The method used depends on the server settings specified in the
/// `ServerParams` instance provided.
///
/// # Arguments
///
/// * `server_params` - An instance of `ServerParams` that contains the settings
///   for the server.
/// * `server_handle_transmitter` - An optional sender channel of type
///   `mpsc::Sender<ServerHandle>` that can be used to manage server state.
///
/// # Errors
///
/// This function will return an error if any of the server starting methods
/// fails.
...
///
/// This function will instantiate and prepare the Findex server and run it on a
...
async fn start_plain_http_findex_server(
    ...
    let findex_server = Arc::new(FindexServer::instantiate(server_params).await?);
...
/// - The SSL acceptor cannot be created or configured with the certificate and
///   key
/// - The Findex server cannot be instantiated or prepared
/// - The server fails to run
...
/**
 * This function prepares a server for the application. It creates an
 * `HttpServer` instance, configures the routes for the application, and
 * sets the request timeout. The server can be configured to use OpenSSL for
 * SSL encryption by providing an `SslAcceptorBuilder`.
 *
 * # Arguments
 *
 * * `findex_server`: A shared reference to the `Findex server` instance to
 *   be used by the application.
 * * `builder`: An optional `SslAcceptorBuilder` to configure the SSL
 *   encryption for the server.
 *
 * # Returns
 *
 * Returns a `Result` type that contains a `Server` instance if successful,
 * or an error if something went wrong.
 *
 * # Errors
 *
 * This function can return the following errors:
 * - `FindexServerError::ServerError` - If there is an error in the server
 *   configuration or preparation.
 */
pub(crate) async fn prepare_findex_server(
    ...
    // Prepare the JWT configurations and the JWKS manager if the server is using
    ...
    let server = HttpServer::new(move || {
        ...
        // The default scope serves from the root
        let default_scope = web::scope("")
            ...
            // Findex endpoints
            .service(findex_batch_read)
            .service(findex_guarded_write)
