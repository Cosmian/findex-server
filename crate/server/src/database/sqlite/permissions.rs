...
impl PermissionsTrait for _Sqlite<CUSTOM_WORD_LENGTH> {
    ...
    async fn create_index_id(&self, user_id: &str) -> FResult<Uuid> {
        ...
        self.pool
            .conn_mut(move |conn| {
                conn.execute(
                "INSERT INTO findex.permissions (user_id, index_id, permission) VALUES (?1, ?2, ?3)",
    ...
    async fn set_permission(
        ...
        .conn_mut(move |conn| {
            conn.execute(
                "INSERT OR REPLACE INTO findex.permissions (user_id, index_id, permission) VALUES (?1, ?2, ?3)",
    ...
    async fn get_permission(&self, user_id: &str, index_id: &Uuid) -> FResult<Permission> {
        ...
        let permission = self
            .pool
            .conn(move |conn| {
                let query = "SELECT permission FROM findex.permissions WHERE user_id = ?1 AND index_id = ?2";
    ...
    async fn get_permissions(&self, user_id: &str) -> FResult<Permissions> {
        ...
        let red_permissions = self
            ...
            .conn(move |conn| {
                let query =
                    "SELECT index_id,permission  FROM findex.permissions WHERE user_id = ?1";
                let mut stmt = conn.prepare(&query)?;
    ...
    async fn revoke_permission(&self, user_id: &str, index_id: &Uuid) -> FResult<()> {
        ...
        self.pool
            .conn_mut(move |conn| {
                conn.execute(
                    "DELETE FROM findex.permissions WHERE user_id = ?1 AND index_id = ?2",
