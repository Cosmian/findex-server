...
use super::{types::LoginResult, jwks::JwksManager};
...
impl JwtConfig {
    ...
    pub(crate) fn decode_authentication_token(&self, token: &str) -> LoginResult<UserClaim> {
        assert!(
            !token.is_empty(),
            "token is empty"
            // LoginError::Unauthorized(("token is empty".to_owned()})
        );
        tracing::trace!(
        ...
        let kid = token_kid(token)
            ...
            .ok_or_else(|| LoginError::Unauthorized("No 'kid' claim present in token".to_owned()))?;
