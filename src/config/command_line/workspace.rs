...
use crate::{findex_server_error, result::FResult};

const DEFAULT_ROOT_DATA_PATH: &str = "./cosmian-findex-server";
const DEFAULT_TMP_PATH: &str = "/tmp";

#[derive(Debug, Args, Deserialize, Serialize)]
#[serde(default)]
pub struct WorkspaceConfig {
    /// The root folder where the Findex server will store its data
    /// A relative path is taken relative to the user HOME directory
    #[clap(long, env = "FINDEX_SERVER_ROOT_DATA_PATH", default_value = DEFAULT_ROOT_DATA_PATH)]
    ...
    #[clap(long, env = "FINDEX_SERVER_TMP_PATH", default_value = DEFAULT_TMP_PATH)]
...
impl WorkspaceConfig {
    pub(crate) fn init(&self) -> FResult<Self> {
    ...
    pub(crate) fn finalize_directory(&self, path: &PathBuf) -> FResult<PathBuf> {
    ...
    pub(crate) fn finalize_directory_path(
        ...
        fs::canonicalize(path).map_err(|e| findex_server_error!(e))
    }
    ...
    #[allow(dead_code)]
    pub(crate) fn finalize_file_path(&self, path: &PathBuf) -> FResult<PathBuf> {
        let path = if path.is_relative() {
